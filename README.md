...
3. **Testes internos** — copia `atividades/<nome-da-atividade>/testcase.yaml`
   pro repo do aluno e roda `west twister`.
...
outro tipo de harness do Twister) que valida o comportamento esperado. Não
precisa mexer no código do aluno nem no workflow.
